...
# Création d'une classe Joueur -> name, main, blackjack
...
def jeu():
    ...
    def aff_main(id_joueur, nbr_joueurs_r):
        ...
        if id_joueur == 0:
            print("\n\tMain du dealer : " +
                  str(liste_joueurs[id_joueur].main[0]) + " | [Deuxième carte cachée]")

        else:

            for f in range(nbr_joueurs_r):
                print("Main " + str(liste_joueurs[f+1].name) + ": " + str(liste_joueurs[f+1].main).replace(
                    "'", "") + "\t\t| TOTAL : " + str(total(liste_joueurs[f+1].main)))

    # Fonction permettant d'afficher toutes les cartes du dealer
    def aff_main_dealer_final():
        print("\n\tMain du dealer : " + str(liste_joueurs[0].main).replace(
            "'", "") + "\t\t| TOTAL : " + str(total(liste_joueurs[0].main)) + "\n\n")
            ...
            if changement_liste == 1:
                ...
                for u in range(nbr_joueurs_r):
                    ...
                    if liste_joueurs[u+1].blackjack == 1:
                        ...
                        print("\n\tRetrait du joueur vainqueur en cours : " +
                              liste_joueurs[u+1].name + "\n\n")
        ...
        for b in range(nbr_joueurs_r):
            ...
            while c != 0:
                ...
                if choix == "o":
                    ...
                    print("\n\n" + str(liste_joueurs[b+1].name) + ", Vous terminez la partie avec un score de " + str(
                        total(liste_joueurs[b+1].main)))

                    if b+1 < nbr_joueurs_r:
                        print("\n\n\t\tJoueur suivant : " +
                              str(liste_joueurs[b+2].name) + "\n\n")
